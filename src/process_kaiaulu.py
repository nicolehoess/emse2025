...
# This script matches identities and slices the file-based git log table
# extracted by Kaiaulu for subsequent analyses.
...
def parse_timezone(date_str):
    """
    Helper function to convert invalid time zone offsets to UTC.
    ...
    Returns:
        The date string converted to UTC if necessary.
    """
    try:
...
def read_gitlog(git_log_path):
    ...
    # Some repositories may use invalid timezone offsets,
    # which we naively consider as UTC.
    df["author_datetimetz"] = df["author_datetimetz"].apply(parse_timezone)
    df["committer_datetimetz"] = df["committer_datetimetz"].apply(parse_timezone)
    ...
    try:
        ...
        # If the date string does not match the format above, it may already
...
def create_git_slices(date_list,
    ...
    for range_dir in sorted(os.listdir(entities_path)):
        ...
        # This corresponds to Kaiaulu's replication configuration.
...
def main(project,
    ...
    if time_slice:
        ...
        create_git_slices(date_list, df_filtered, entities_path,
                          file_dir_path)
